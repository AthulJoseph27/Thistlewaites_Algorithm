...
{
    // Blue and Green will be either on the Blue or Green Face
    // Putting all other colors in opposite sides
    ...
    for (auto i : moves)
    {
        ...
        if (validSolution(algo + i))
        {
            ...
            if (mp.find(id) == mp.end() || mp[id].size() > (algo.size() + 1))
            {
                ...
                if (cnt > 100'00'000'000)
                {
                    cnt = 0;
                    string sol_name = "Sol_Checkpoint";
                    string state_name = "States_Checkpoint";
                    sol_name = sol_name + to_string(checkPoint) + ".txt";
                    state_name = state_name + to_string(checkPoint) + ".txt";
                    sol_file.open(sol_name, ios::out);
                    state_file.open(state_name, ios::out);
                    ...
                    cout<<mp.size() <<" Checkpoint" << checkPoint << '\n';
                    checkPoint++;
                }
            }
        }
...
int main()
{
    ...
    sol_file.open("Phase_3A_sol.txt", ios::out);
    state_file.open("Phase_3A_states.txt", ios::out);
